...
chrome.storage.local.get(['modeType', 'folderName'], (results) => {
  if (results.modeType)
    handleChangeMode(
      (results.modeType as (typeof modeType)[number]) || defaultMode
    )
...
function getProgress(downloadId: number, callback: (percent: number) => void) {
    ...
    chrome.storage.local.get(null, (results) => {
      ...
      if (results) {
        ...
        if (
          results?.remapList?.[0] &&
          !Array.isArray(
            (results as SettingsType)?.remapList?.[0]?.item?.replace
          )
        )
          results.remapList = masterSettings.remapList
        if (!results.limitBySelector)
          results.limitBySelector = masterSettings.limitBySelector
      }
      ...
      if (
        !results.modeType ||
        !modeTypes.includes(results.modeType as (typeof modeTypes)[number])
      ) {
        chrome.storage.local.set({
          ...defaultSettings,
          remapList: masterSettings.remapList,
